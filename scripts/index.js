document.addEventListener("DOMContentLoaded", () => {
  ...
  document.addEventListener("keydown", (event) => {
    if (
      event.key === "Escape" &&
      cartDrawer &&
      cartDrawer.classList.contains("cart-open")
    ) {
      closeCartDrawer();
    }
  });
  ...
  function updateCheckoutLink() {
    ...
    if (cart.length === 0) {
      ...
      checkoutButton.style.opacity = "0.55";
    ...
    let message = "Hello MedVanceNg,%0A%0A";
    ...
    cart.forEach((item) => {
      ...
      message += `• ${encodeURIComponent(item.name)} x${item.quantity} - ${encodeURIComponent(formatCurrency(itemTotal))}%0A`;
    });

    message += `%0ATotal: ${encodeURIComponent(formatCurrency(total))}`;

    message += "%0A%0APlease let me know the next steps.";

    checkoutButton.href = `https://wa.me/${WHATSAPP_NUMBER}?text=${message}`;
